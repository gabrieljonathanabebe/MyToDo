...
def from_storage(df: pd.DataFrame) -> list[Task]:
    ...
    for raw_row in raw_rows:
        row = {
            key: value if pd.notna(value) else None for key, value in raw_row.items()
        }
...
def to_storage(tasks: list[Task]) -> pd.DataFrame:
    ...
    for task in tasks:
        ...
        serialized = {field: serialize_value(value) for field, value in base.items()}
